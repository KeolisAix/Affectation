...
function getServiceSQL($pdoAffectation, $periode, $JourDeAffectation)
{
    ...
    $sql = "SELECT DISTINCT * FROM \"public\".\"base\" WHERE periode = '".$periode."' AND \"" . $jourlowcase ."\" = 'O' ORDER BY \"public\".base.code_tache ASC, arret_debut_heure ASC limit 2";    
...
function getSprinterSQL($pdoVehicule)
{
    $sql = 'SELECT vehicules.vehicule.parc_keolis, vehicules.vehicule.modele  FROM vehicules.vehicule WHERE vehicules.vehicule.modele = \'SPRINTER\'';  
    $req = $pdoVehicule->query($sql);
    while ($row = $req->fetch()) {
        echo '<div id="'.$row["parc_keolis"].'" class="redips-drag ui-widget-content" onmouseout="testdrop(\''.$row["parc_keolis"].'\')" style="width: 695px, position: relative; background-color: lightgrey">'.$row["parc_keolis"].'</div>';
    }
